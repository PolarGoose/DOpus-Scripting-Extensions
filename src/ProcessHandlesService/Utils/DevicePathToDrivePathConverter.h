...
using DeviceNameToDriveLetterMap = std::map<std::wstring, wchar_t>;
...
//     {"\Device\CdRom0\"                                          , 'X'} ]
inline DeviceNameToDriveLetterMap CreateDeviceNameToDriveLetterConversionMap() {
  DeviceNameToDriveLetterMap conversionMap;
  ...
  for (auto driveLetter = L'A'; driveLetter <= L'Z'; driveLetter++) {
    ...
    conversionMap.emplace(std::wstring{ deviceNameBuf.data() } + L'\\', driveLetter);
  }
...
inline std::wstring GetDriveLetterBasedFullName(const DeviceNameToDriveLetterMap& conversionMap, std::wstring_view deviceBasedFileFullName) {
  for (const auto& [deviceName, driveLetter] : conversionMap) {
  ...
  THROW_WEXCEPTION(L"Couldn't convert '{}' to the drive letter based path", deviceBasedFileFullName);
}
